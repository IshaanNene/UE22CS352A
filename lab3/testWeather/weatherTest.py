...
try:
    ...
    print("Rename your written program as  CAMPUS_SECTION_SRN_Lab3.py and run python weatherTest.py --ID CAMPUS_SECTION_SRN_Lab1 --data weatherData.csv")
    sys.exit()
...
def test_case():
    ...
    try:
        ...
        dictionary = ans[0]
        flag = (
            (dictionary[0] >= 0.350 and dictionary[0] <= 0.365)
            and (dictionary[1] >= 0.0119 and dictionary[1] <= 0.0131)
            and (dictionary[2] >= 0.059 and dictionary[2] <= 0.062)
            and (dictionary[3] >= 0.007 and dictionary[3] <= 0.0076)
            and (ans[1] == 0)
        )
        if flag:
            print("Test Case 4 for the function get_selected_attribute PASSED")
        else:
            print("Test Case 4 for the function get_selected_attribute FAILED",ans)
    except:
